...
pub fn render_requests(
    ...
    selected_idx: usize,
    flash_state: Option<(usize, FlashKind, u64)>,
) {
    let chunks = Layout::vertical([Constraint::Min(0), Constraint::Length(6)])
        .split(area);

    let _header = Row::new(vec!["  ", "Hostname", "IP", "Source", "Age"])
    ...
    let rows: Vec<Row> = requests
        ...
        .enumerate()
        .map(|(i, r)| {
            let hostname = truncate(&r.hostname, 24);
            let source = r
                .source_ip
                .as_ref()
                .map(|s| truncate(s, 16))
            ...
            let style = if let Some((idx, kind, _)) = flash_state {
                if i == idx {
                    flash_style(kind)
    ...
    if let Some(req) = requests.get(selected_idx) {
        ...
        Card::new(&format!("Request: {}", req.hostname)).render(
            frame,
            chunks[1],
            vec![
                Line::from(vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled("  [a]pprove  [d]eny", DARK_THEME.muted),
                ]),
            ],
        );
    }
...
fn flash_style(kind: FlashKind) -> Style {
    match kind {
        FlashKind::Approve => Style::default().fg(DARK_THEME.accent).bg(DARK_THEME.bg),
        FlashKind::Deny => Style::default().fg(DARK_THEME.danger).bg(DARK_THEME.bg),
    }
}
