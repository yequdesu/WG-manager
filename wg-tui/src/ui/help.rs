...
pub fn render_help(frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        help_line("v", "View invite URL and command"),
        help_line("F", "Force-delete invite after confirmation"),
        ...
        Line::from(Span::styled(
            "  WG-TUI v0.2.0  ·  Ratatui Dashboard",
            DARK_THEME.muted,
        )),
    ];

    frame.render_widget(
        Paragraph::new(lines).style(Style::default().fg(DARK_THEME.text)),
        inner,
    );
}
