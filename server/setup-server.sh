...
print_summary() {
    ...
    echo -e "    ${BOLD}curl -o wg0.conf \"http://${SERVER_PUBLIC_IP}:${mgmt_port}/api/v1/windows-config?name=%COMPUTERNAME%\"${NC}"
    echo ""
    echo -e "  ${BOLD}${CYAN}Windows (PowerShell):${NC}"
    echo -e "    ${BOLD}Invoke-WebRequest \"http://${SERVER_PUBLIC_IP}:${mgmt_port}/api/v1/windows-config?name=\$env:COMPUTERNAME\" -OutFile wg0.conf${NC}"
    echo ""
